...
# Variável global para controlo por visão
_controlo_visao_ativo = False
_funcao_deve_saltar = None
_funcao_direcao_horizontal = None


def ativar_controlo_visao(funcao_saltar, funcao_direcao=None):
    """Ativa o controlo por visão computacional."""
    global _controlo_visao_ativo, _funcao_deve_saltar, _funcao_direcao_horizontal
    _controlo_visao_ativo = True
    _funcao_deve_saltar = funcao_saltar
    _funcao_direcao_horizontal = funcao_direcao
...
def iniciar_jogo():
    ...
    def coin_animation():
        nonlocal coin_surface, coin_index
        coin_index += 0.1
    ...
    def primary_snail_animation():
        nonlocal snail_surface, snail_index
        snail_index += 0.1
    ...
    screen.fill("Green")
    pygame.display.set_caption("Infinite Runner - Controlo por Visao")
    ...
    # Variáveis para movimento lateral
    player_speed_x = 0
    player_max_speed = 5
    player_x_position = 50  # Posição X inicial
    ...
    player_rect = player_surface.get_rect(bottomleft=(player_x_position, 300))
    ...
    # Variável para controlar o cooldown do salto
    ultimo_salto = 0
    cooldown_salto = 500
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if game_running == True:
                ...
                # Controlo por teclado (mantido para testes)
                if event.type == pygame.KEYDOWN and player_rect.colliderect(snail_rect) == False:
                    if event.key == pygame.K_UP and player_rect.bottom >= 100:
                        ...
                        gravity = 10
                    # Adicionar controlo lateral por teclado
            ...
            if game_running == False:
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_SPACE:
                        ...
                        player_x_position = 50  # Reset posição
        ...
        # Controlo por visão computacional (FASE 1)
        if game_running and _controlo_visao_ativo:
            # Controlo de salto
            if _funcao_deve_saltar and _funcao_deve_saltar() and player_rect.bottom >= 300:
                if tempo_atual - ultimo_salto > cooldown_salto:
                    gravity = -18
                    ultimo_salto = tempo_atual

            # Controlo de movimento lateral
            if _funcao_direcao_horizontal:
                direcao = _funcao_direcao_horizontal()
        ...
        if game_running == True:
            ...
            if _controlo_visao_ativo:
                vision_font = pygame.font.Font(None, 20)
                vision_text = vision_font.render("Controlo por Visao Ativo", True, "yellow")
                screen.blit(vision_text, (30, 40))
            ...
            player_x_position += player_speed_x

            # Limitar movimento dentro da tela
            if player_x_position < 10:
                player_x_position = 10
            elif player_x_position > 750:
